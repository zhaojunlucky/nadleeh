...
func TestSdtOutputWriter_String(t *testing.T) {
	...
	t.Run("BufferWithData", func(t *testing.T) {
		...
		if result != testData {
			t.Errorf("Expected buffer to contain %q, got %q", testData, result)
		}
	})
...
func TestSdtOutputWriter_Masking(t *testing.T) {
	t.Run("MasksSensitiveValues", func(t *testing.T) {
		writer := NewStdOutputWriter(NewOutputMasker(map[string]string{
			"API_TOKEN": "super-secret-token",
			"NORMAL":    "visible",
		}))

		_, err := writer.Write([]byte("token=super-secret-token normal=visible"))
		if err != nil {
		...
		if strings.Contains(result, "super-secret-token") {
			t.Errorf("Expected secret to be masked, got %q", result)
		}
		if !strings.Contains(result, "token=***") {
			t.Errorf("Expected masked token, got %q", result)
		}
		if !strings.Contains(result, "normal=visible") {
			t.Errorf("Expected non-sensitive value to remain, got %q", result)
		}
	})
}
