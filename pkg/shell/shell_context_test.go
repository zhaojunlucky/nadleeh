...
func TestShellContext_Run(t *testing.T) {
	...
	t.Run("SimpleEchoWithOutput", func(t *testing.T) {
		...
		if !strings.Contains(output, "Hello, World!") {
			t.Errorf("Expected captured output to contain greeting, got %q", output)
		}
	})
	...
	t.Run("PreservesExitCode", func(t *testing.T) {
		script := "exit 42"
	...
	t.Run("EnvironmentVariables", func(t *testing.T) {
		...
		if !strings.Contains(output, "TEST_VAR=test_value ANOTHER_VAR=another_value") {
			t.Errorf("Expected captured environment output, got %q", output)
		}
	})

	t.Run("MasksSensitiveEnvironmentVariables", func(t *testing.T) {
		...
		if strings.Contains(output, "super-secret-token") {
			t.Errorf("Expected secret to be masked, got %q", output)
		}

		if !strings.Contains(output, "API_TOKEN=***") {
			t.Errorf("Expected masked token output, got %q", output)
		}
	})
	...
	t.Run("ScriptWithStderr", func(t *testing.T) {
		...
		if !strings.Contains(output, "error message") {
			t.Errorf("Expected captured stderr output, got %q", output)
		}
	})

	t.Run("Timeout", func(t *testing.T) {
		timeoutCtx := NewShellContext()
		timeoutCtx.Timeout = 50 * time.Millisecond

		exitCode, _, err := timeoutCtx.Run(newMockEnv(), "sleep 1", true)
...
func TestShellContext_Integration(t *testing.T) {
	...
	t.Run("CompileAndRun", func(t *testing.T) {
		...
		if !strings.Contains(output, "Integration test") {
			t.Errorf("Expected captured integration output, got %q", output)
		}
	})
...
func TestShellContext_EdgeCases(t *testing.T) {
	...
	t.Run("LongScript", func(t *testing.T) {
		...
		if !strings.Contains(output, "start") || !strings.Contains(output, "end") {
			t.Errorf("Expected captured long script output, got %q", output)
		}
	})
}
