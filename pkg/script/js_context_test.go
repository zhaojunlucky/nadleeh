...
func TestNewJSContext_NilSecureContext(t *testing.T) {
	jsCtx := NewJSContext(nil)

	if jsCtx.JSSecCtx.secureCtx == nil {
		t.Fatal("Expected secure context to be initialized")
	}

	if jsCtx.JSSecCtx.IsEncrypted("plain text") {
		t.Error("Expected plain text to not be encrypted")
	}
}
...
func TestJSContext_Run(t *testing.T) {
	...
	t.Run("ConsoleOutputCaptured", func(t *testing.T) {
		script := "console.log('hello from js'); 'done';"
		variables := map[string]interface{}{}
		...
		if !strings.Contains(output, "hello from js") {
			t.Errorf("Expected console output to be captured, got %q", output)
		}

		if !strings.Contains(output, "done") {
			t.Errorf("Expected return value to be captured, got %q", output)
		}
	})

	t.Run("MasksSensitiveConsoleOutput", func(t *testing.T) {
		mockEnv.Set("API_TOKEN", "super-secret-token")
		script := "console.log(secret); secret;"
		variables := map[string]interface{}{
		...
		if strings.Contains(output, "super-secret-token") {
			t.Errorf("Expected token to be masked, got %q", output)
		}

		if !strings.Contains(output, "***") {
			t.Errorf("Expected masked token in output, got %q", output)
		}
	})

	t.Run("Timeout", func(t *testing.T) {
		timeoutCtx := NewJSContext(&encrypt.SecureContext{})
		timeoutCtx.Timeout = 50 * time.Millisecond

		exitCode, _, err := timeoutCtx.Run(mockEnv, "while (true) {}", map[string]interface{}{})
